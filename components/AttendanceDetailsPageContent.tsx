...
export function AttendanceDetailsPageContent() {
  ...
  const processRfidTap = useCallback(async (scannedValue: string) => {
    const scannedRfid = scannedValue.trim();
    ...
    try {
      ...
      setScanMessage(error instanceof Error ? error.message : "Unable to record RFID tap.");
    } finally {
